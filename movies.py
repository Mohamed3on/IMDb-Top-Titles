...
MINVOTES = str(80000)
MAXVOTES = str(800000)
MIN_RELEASE_DATE = str(1973)
MAX_RUNTIME = str(200)  # at most as long as seven samurai
MIN_RATING = str(7.5)

URL = (
    "http://www.imdb.com/search/title?count=250&num_votes="
    + MINVOTES
    + ","
    + MAXVOTES
    + "&sort=num_votes,desc&view=simple&my_ratings=exclude&user_rating="
    + MIN_RATING
    + ",&release_date="
    + MIN_RELEASE_DATE
    + ",&runtime=,"
...
def get_stuff(url, filename="sortedtitles"):
    ...
    SCORES = commonfunctions.loadfile("scores")
    SCORES = {k: v for (k, v) in SCORES.items() if v[0] > MINSCORE and v[2] > MIN_RATIO}
